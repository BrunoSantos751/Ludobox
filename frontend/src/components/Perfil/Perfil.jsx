...
function Profile({ userId: loggedInUserId, username: propUsername }) {
  ...
  const refreshFollowCounts = async () => {
    ...
    if (loggedInUserId) {
      try {
        const followersResponse = await fetch(`http://localhost:8080/api/users/${loggedInUserId}/seguidores`);
        ...
        const followingResponse = await fetch(`http://localhost:8080/api/users/${loggedInUserId}/seguindo`);
  ...
  useEffect(() => {
    if (loggedInUserId) {
      ...
      const fetchUserProfile = async (idToFetch) => {
        try {
          const response = await fetch(`http://localhost:8080/api/users/${idToFetch}/profile`);
          if (!response.ok) {
      ...
      const fetchUserGamesByStatus = async (idToFetch) => {
        try {
          const response = await fetch(`http://localhost:8080/api/users/${idToFetch}/games_by_status`);
          if (!response.ok) {
      ...
      const fetchUserReviews = async (idToFetch) => {
        try {
          const response = await fetch(`http://localhost:8080/api/users/${idToFetch}/reviews`);
          if (!response.ok) {
  ...
  const handleSaveBio = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:8080/api/users/${userData.userId}/profile`, {
        method: 'PATCH',
        headers: {
  ...
  const handleSearchGames = async (e) => {
    ...
    if (e.target.value.length > 2) {
      try {
        const response = await fetch(`http://localhost:8080/api/games?search=${encodeURIComponent(e.target.value)}&page_size=5`);
        if (!response.ok) {
  ...
  const handleSaveGame = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:8080/api/users/${loggedInUserId}/games`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      const gamesResponse = await fetch(`http://localhost:8080/api/users/${loggedInUserId}/games_by_status`);
      if (!gamesResponse.ok) {
  ...
  const handleDeleteGame = async (gameName) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:8080/api/users/${loggedInUserId}/games/${encodedGameName}`, {
        method: 'DELETE',
        credentials: 'include'
      });
  ...
  const handleDeleteReview = async (reviewId) => {
    ...
    try {
      const url = `http://localhost:8080/api/users/${loggedInUserId}/reviews/${reviewId}`;
      ...
      const reviewsResponse = await fetch(`http://localhost:8080/api/users/${loggedInUserId}/reviews`);
